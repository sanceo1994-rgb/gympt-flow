...
export const Route = createFileRoute("/demo/trainer")({
  head: () => ({
    meta: [
      { title: "트레이너 화면 체험 — 픽짐피티 PickGymPT" },
      {
        name: "description",
        content:
          "실제 트레이너 일정 조율 화면을 가상 데이터로 그대로 체험해보세요. 로그인 없이 자유롭게 눌러볼 수 있어요.",
      },
    ],
  }),
  component: DemoTrainerSchedule,
});
...
function DemoTrainerSchedule() {
  ...
  // Demo page: no auth, no Supabase. Seed everything synchronously from the
  // hardcoded dummy data below, but still run it through the real AI matching
  // algorithm so the "AI 추천 결과" panel behaves identically to production.
  useEffect(() => {
    setTrainerName("김트레이너");

    const ids: Record<string, string> = {};
    for (let day = 0; day < 7; day++) {
      for (const hour of HOURS) ids[`${DAYS[day]}-${hour}`] = `${DAYS[day]}-${hour}`;
    }
    setSlotIds(ids);

    const matchingSlots: MatchingSlot[] = DAYS.flatMap((day) =>
      HOURS.map((hour) => ({
        id: `${day}-${hour}`,
        day,
        hour,
        capacity: 1,
        isClosed: closed.has(`${day}-${hour}`),
      })),
    );
    const matching = maximizeScheduleAssignments(
      DEMO_STUDENTS.filter((student) => student.status === "응답완료").map((student) => ({
        id: student.name,
        name: student.name,
    ...
    const result = matching.assignments.map((assignment) => ({
      day: assignment.day,
      hour: `${String(assignment.hour).padStart(2, "0")}:00`,
      name: assignment.studentName,
    }));

    setDbStudents(DEMO_STUDENTS);
    setDbPicks(DEMO_PICKS);
    ...
    setPanelSelected(new Set(DEMO_STUDENTS.map((student) => student.name)));
    setDbActivity(DEMO_ACTIVITY);
    // weekOffset intentionally omitted: the demo always shows the same dummy week.
  ...
  const applyPending = async () => {
    ...
    // Demo page — visually identical interaction, but nothing is persisted.
    const parts: string[] = [];
    if (toClose) parts.push(`${toClose}개 시간을 닫았어요`);
    if (toOpen) parts.push(`${toOpen}개 시간을 다시 열었어요`);
    fireToast((parts.join(" · ") || "변경사항 저장됨") + " (체험 모드 · 실제로 저장되지는 않아요)");
  };
  ...
  const displayTrainerName = trainerName || "담당";
  ...
  return (
    <AppShell>
      ...
      {/* Floating invite bar — primary CTA, hidden when pendingClose bar overlays it.
          Demo-only: each week shows a different stage of the real flow (확정 / 진행 중 / 요청 보내기)
          so visitors can see all three without needing real data. */}
      {pendingClose.size === 0 && (
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-[7px] text-[14.5px] font-bold leading-tight tracking-tight text-white sm:text-[16px]">
                  <span className="sm:hidden">{WEEK_LABELS[weekOffset]} 응답 현황 · {STUDENTS.length}명 발송됨</span>
...
function ToastHourglassIcon() {
  return (
    <span className="relative shrink-0 h-[30px] w-[30px] animate-hourglass-flip" style={{ filter: "drop-shadow(0 3px 4px rgba(0,0,0,.3))" }}>
      ...
      />
      <span
        className="absolute left-[8px] top-[2px] h-[2.5px] w-[14px] rounded-full"
        style={{ background: "linear-gradient(160deg, #c47a08, #8a5300)" }}
      />
      <span
        className="absolute left-[8px] top-[23.5px] h-[2.5px] w-[14px] rounded-full"
        style={{ background: "linear-gradient(160deg, #c47a08, #8a5300)" }}
      />
    </span>
  );
