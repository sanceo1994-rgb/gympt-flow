alter table public.student_selections
  add column if not exists assigned_method text not null default 'auto'
    check (assigned_method in ('auto', 'manual'));

drop function if exists public.confirm_weekly_schedule(uuid, jsonb);
...
begin
  ...
  using jsonb_array_elements(p_assignments) as rec
